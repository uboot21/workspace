...
LAN=vmbr0
# Name des VLAN (keine VLAN -> Unten Variable (,tag="$TAG") entfernen)
TAG=50
...
virt-customize -a "$FILE2" --run-command "chown -R "$User":"$User" /home/"$User""

qm create "$Prox_ID" --name "ubuntu-2204-cloudinit-template" --memory 2048 --cores 2 --scsihw virtio-scsi-pci --net0 virtio,bridge="$LAN",tag="$TAG"
